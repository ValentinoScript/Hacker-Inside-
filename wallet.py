...
# ------------------ CONFIG Mainnet ------------------
BIP84_PATH = "m/84h/0h/0h/0/0"   # BIP84 Mainnet index 0
UTXO_API = "https://mempool.space/api/address/{addr}/utxo"
BROADCAST_API = "https://mempool.space/api/tx"
WALLETS_FILE = "wallets_mainnet.json"
# Default fees / dust (satoshis)
DEFAULT_FEE_SAT = 5000     # 0.00005 BTC
DEFAULT_DUST = 1000
...
def _p2pkh_scriptcode(h160: bytes) -> bytes:
    # scriptCode for P2WPKH per BIP143
...
def _op_return_script(data: bytes) -> bytes:
    ...
    if len(data) <= 75:
        ...
        push = b'\x4c' + bytes([len(data)])  # OP_PUSHDATA1
...
# ------------------ Bech32 decode minimal (support v0 addresses bc1/tb1) ------------------
...
def address_to_spk(address: str) -> bytes:
    """
    Retourne le scriptPubKey bytes pour une adresse bech32 v0 (bc1/tb1).
    Supporte uniquement witness v0 (P2WPKH / P2WSH).
    """
    addr = address.lower()
    if addr.startswith("bc1") or addr.startswith("tb1"):
        ...
        raise ValueError("Seules les adresses bech32 (bc1...) sont supportées pour l'envoi actuellement.")
...
def derive_key0_from_mnemonic(phrase: str) -> HDKey:
    ...
    return HDKey(import_key=k0.private_hex, network='bitcoin', witness_type='segwit')

def derive_key0_from_wif(wif: str) -> HDKey:
    return HDKey(import_key=wif, network='bitcoin', witness_type='segwit')
...
def wif_from_private_hex(priv_hex: str) -> str:
    try:
        k = Key(import_key=priv_hex, network='bitcoin')
        return k.wif()
    except Exception:
        return None
...
def show_utxos_and_balance_for_key(key0: HDKey):
    ...
    print(f"💰 Solde (incl. mempool) : {total/1e8:.8f} BTC — {len(utxos)} UTXO(s)")
    for u in utxos:
        st = "✔" if u.get("status", {}).get("confirmed") else "🕓"
        print(f" • {u['value']/1e8:.8f} BTC — {u['txid']}:{u['vout']} ({st})")
    return utxos
...
# ------------------ Main CLI ------------------
def main():
    print("=== Wallet Mainnet (SegWit) — CLI complet ===")
    active_key = None
...
""")
        ...
        if choice == '1':
            ...
            if sub == 'a':
                ...
                wif = input("Clé privée WIF (mainnet) : ").strip()
                key0 = derive_key0_from_wif(wif)
                secret_type, secret = "wif", wif
            else:
                xprv = input("Clé étendue xprv (mainnet) : ").strip()
...
--- Wallet actif [{addr}] ---
a) Solde & UTXO
b) Envoyer BTC (bech32 dest uniquement)
...
""")
            ...
            if op == 'a':
                ...
                dest = input("Adresse destinataire (bc1...) : ").strip()
                amt = float(input("Montant (BTC) : ").strip())
                fee = input(f"Frais (BTC) [défaut {DEFAULT_FEE_SAT/1e8:.8f}] : ").strip()
